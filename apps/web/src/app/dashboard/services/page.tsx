...
export default async function ServicesListPage({
    ...
    const [{ data: branches }, { data: services, error }] = await Promise.all([
        supabase.from('branches')
            .select('id,name')
            .eq('biz_id', bizId).eq('is_active', true).order('name'),
    ...
    const list = (services ?? []).filter(s => !branchFilter || s.branch_id === branchFilter);

    return (
        <main className="mx-auto max-w-6xl space-y-4">
            <div className="flex items-center justify-between">
                <h1 className="text-2xl font-semibold">Услуги</h1>
                <Link href="/dashboard/services/new" className="border rounded px-3 py-1">+ Добавить</Link>
            </div>

            <div className="flex flex-wrap gap-2 items-center">
                <span className="text-sm text-gray-600">Филиал:</span>
                <Link
                    className={`text-sm px-2 py-1 rounded border ${branchFilter ? 'hover:bg-gray-50' : 'bg-gray-100 font-medium'}`}
                ...
                {(branches ?? []).map((b: Branch) => (
                    <Link
                        key={b.id}
                        className={`text-sm px-2 py-1 rounded border ${branchFilter === b.id ? 'bg-gray-100 font-medium' : 'hover:bg-gray-50'}`}
            ...
            <div className="border rounded overflow-x-auto">
                <table className="min-w-full text-sm">
                    ...
                    {list.map((s: ServiceRow) => {
                        ...
                        return (
                            <tr key={s.id} className="border-t">
                                ...
                                <td className="p-2">{s.price_from}–{s.price_to}</td>
                                <td className="p-2">{branchName}</td>
                                <td className="p-2">{s.active ? 'активна' : 'скрыта'}</td>
                                <td className="p-2">
                                    <div className="flex gap-2">
                                        <Link href={`/dashboard/services/${s.id}`} className="border rounded px-2 py-1">Редакт.</Link>
                    ...
                    {list.length === 0 && (
                        <tr><td className="p-3 text-gray-500" colSpan={6}>Нет услуг</td></tr>
                    )}
                    </tbody>
                </table>
            </div>
