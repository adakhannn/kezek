...
export function useShiftItems({ 
    ...
    useEffect(() => {
        ...
        // (имеют id ИЛИ bookingId ИЛИ суммы больше 0 ИЛИ заполнено имя клиента, но не автоматически сгенерированное)
        const itemsToSave = items.filter(it => {
            ...
            // Если заполнено имя клиента, но не автоматически сгенерированное "Клиент N" - сохраняем
            if (it.clientName && it.clientName.trim() !== '') {
                const trimmedName = it.clientName.trim();
                // Проверяем, не является ли это автоматически сгенерированным именем
                // Паттерн: "Клиент N", "Client N", "Клиент1", "Client1" и т.д.
                const autoGeneratedPattern = /^(Клиент|Client)\s*\d+$/i;
                if (!autoGeneratedPattern.test(trimmedName)) {
                    return true; // Это реальное имя клиента, сохраняем
                }
                // Если это "Клиент N", но есть другие данные (serviceName или суммы), сохраняем
                if ((it.serviceName && it.serviceName.trim() !== '') || 
                    (it.serviceAmount ?? 0) > 0 || 
                    (it.consumablesAmount ?? 0) > 0) {
                    return true;
                }
                // Иначе - это пустая форма с автоматически сгенерированным именем, не сохраняем
            }
        ...
        saveTimeoutRef.current = setTimeout(async () => {
            ...
            const currentItemsToSave = items.filter(it => {
                ...
                if (it.clientName && it.clientName.trim() !== '') {
                    ...
                    const autoGeneratedPattern = /^(Клиент|Client)\s*\d+$/i;
                    if (!autoGeneratedPattern.test(trimmedName)) {
                        return true; // Это реальное имя клиента
                    }
                    // Если это "Клиент N", но есть другие данные, сохраняем
                    if ((it.serviceName && it.serviceName.trim() !== '') || 
                        (it.serviceAmount ?? 0) > 0 || 
                        (it.consumablesAmount ?? 0) > 0) {
                        return true;
    ...
    const addClient = async (newItem: Omit<ShiftItem, 'id'>) => {
        ...
        try {
            ...
            // НЕ сохраняем элементы с автоматически сгенерированными именами без дополнительных данных
            const itemsToSave = newItems.filter(it => {
                if (it.id) return true; // Элементы с id всегда сохраняем (уже на сервере)
                if (it.bookingId) return true; // Элементы с bookingId всегда сохраняем
                if ((it.serviceAmount ?? 0) > 0 || (it.consumablesAmount ?? 0) > 0) return true; // Есть суммы - сохраняем
                
                if (it.clientName && it.clientName.trim() !== '') {
                    const trimmedName = it.clientName.trim();
                    const autoGeneratedPattern = /^(Клиент|Client)\s*\d+$/i;
                    
                    // Если имя НЕ автоматически сгенерированное - сохраняем
                    if (!autoGeneratedPattern.test(trimmedName)) {
                        return true;
                    }
                    
                    // Если имя автоматически сгенерированное, проверяем, есть ли дополнительные данные
                    // Сохраняем только если есть услуга, сумма услуги или расходники
                    if ((it.serviceName && it.serviceName.trim() !== '') || 
                        (it.serviceAmount ?? 0) > 0 || 
                        (it.consumablesAmount ?? 0) > 0) {
                        return true;
                    ...
                    // Автоматически сгенерированное имя без дополнительных данных - НЕ сохраняем
                    return false;
                }
                
                // Нет имени клиента - не сохраняем
                return false;
            });
            ...
            // Дедупликация: удаляем items без id, если есть соответствующий item с id
            const deduplicatedItems = itemsToSave.filter((item, index, arr) => {
                if (item.id) {
                    return true;
                }
                const hasDuplicateWithId = arr.some((otherItem) => {
