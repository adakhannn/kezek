    ...
    return withRateLimit(
        ...
        if (targetStaffId) {
            ...
            if (staffCheck.error || !staffCheck.data) {
                ...
                return createErrorResponse('forbidden', 'Сотрудник не принадлежит этому бизнесу', undefined, statusCode);
            }
        ...
        if (findError) {
            ...
            return createErrorResponse('not_found', errorMessage, undefined, statusCode);
        }
        ...
        if (items.length > 0) {
            ...
            if (cleanItems.length > 0) {
                ...
                if (insertError) {
                    ...
                    return createErrorResponse('internal', 'Не удалось сохранить позиции', undefined, 500);
                }
            }
        }
        ...
        // Определяем, какие существующие items нужно удалить
        ...
        // Удаляем только если:
        // 1. Есть items для удаления
        // 2. В запросе есть хотя бы один item с id ИЛИ список пустой (пользователь удалил всех клиентов)
        // Это предотвращает случайное удаление при ошибках синхронизации, но позволяет удалять клиентов
