...
20. ✅ **Улучшение обработки ошибок в booking flow** (Выполнено)
    - ✅ Стандартизирована обработка ошибок в `createBooking` и `createGuestBooking` с использованием `fmtErr` и единых сообщений.
    - ✅ Добавлена retry-логика для сетевых ошибок (helper `withNetworkRetry` для `fetch`-запросов).
    - ✅ Улучшены сообщения об ошибках для пользователя: добавлены более понятные и actionable тексты для типичных сценариев (проблемы с сетью, ошибки RPC, некорректные данные).
    - ✅ Добавлено структурированное логирование ошибок через `logError` с указанием scope (`BookingFlow`, `GuestBooking`, `BookingNotify`).

21. ✅ **Оптимизация загрузки данных** (Выполнено)
    - ✅ Настроен React Query Provider для управления состоянием серверных данных.
    - ✅ Созданы хуки для кэширования данных: `useBranchPromotions`, `useServiceStaff`, `useClientBookings`.
    - ✅ Заменены useEffect на React Query хуки для автоматического кэширования и управления состоянием.
    - ✅ Добавлено кэширование с TTL: 5 минут для основных данных, 2 минуты для промо, 1 минута для бронирований клиента.
    - ✅ Оптимизировано количество запросов: данные загружаются параллельно и кэшируются.
    - ✅ Добавлен prefetching при наведении на кнопку "Записаться" для ускорения перехода.
    - ✅ Автоматическая очистка устаревших данных из кэша.
    - 💡 **Рекомендация:** Рассмотреть использование React Query DevTools для отладки кэша в development режиме.
