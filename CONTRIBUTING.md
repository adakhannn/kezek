...
См. `apps/mobile/README.md` и `ENV_SETUP.md` внутри мобильного приложения.
...
```
  ...
  - ⚠️ **Важно**: Минимальный порог покрытия — **40%** (statements/branches/functions/lines)
  - Если покрытие ниже порога, тесты упадут — нужно добавить тесты
  - По мере развития проекта порог будет постепенно подниматься (50%+, 60%+, и т.д.)

- При изменении критичных флоу (бронь, смены, финансы, промо) — обновить или добавить E2E‑сценарии:
...
- [ ] Линтер и typecheck проходят без ошибок.
- [ ] Unit/integration‑тесты пройдены, критичные E2E обновлены при необходимости.
- [ ] Нет «голых» `console.*` в продакшен‑коде.
- [ ] Все новые/изменённые тексты UI локализованы на KY/RU/EN.
- [ ] UX для операторов/менеджеров проверен на мобильной ширине.
- [ ] Актуализированы связанные MD‑документы.
...
| Покрыт ли **новый/изменённый код тестом** (unit или E2E)? | Web: `pnpm -C apps/web test:coverage` (порог 40%); при изменении критичных флоу — E2E. Mobile: `pnpm -C apps/mobile test` (smoke). |
