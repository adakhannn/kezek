-- Исправление функции get_free_slots_service_day_v2
-- Исключаем истекшие hold бронирования из проверки доступности слотов
        ...
        free AS (
          ...
          WHERE NOT EXISTS (
            ...
            WHERE bk.biz_id = p_biz_id
              ...
              AND bk.status IN ('confirmed', 'paid')  -- Только подтвержденные и оплаченные бронирования
...
COMMENT ON FUNCTION public.get_free_slots_service_day_v2 IS 'Возвращает свободные слоты для услуги на указанный день. Учитывает только подтвержденные (confirmed) и оплаченные (paid) бронирования.';
