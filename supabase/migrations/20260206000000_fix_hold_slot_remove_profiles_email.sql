...
-- Убираем использование колонки email из таблицы profiles
-- Email получаем из auth.users напрямую
...
COMMENT ON FUNCTION public.hold_slot IS 'Создает бронь для авторизованного пользователя сразу со статусом confirmed. Получает email из auth.users, а не из profiles. Учитывает только подтвержденные (confirmed) и оплаченные (paid) бронирования при проверке пересечений. Возвращает ID созданной брони.';
