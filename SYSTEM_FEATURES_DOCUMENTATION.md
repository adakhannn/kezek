...
#### 1.6.5 Долгосрочное здоровье рейтингов

**Стратегия обработки периодов без активности:**

1. **Периоды без метрик (каникулы, простой, неактивность):**
   - Если за день нет активности (нет смен, нет клиентов), метрики для этого дня **не создаются** в `staff_day_metrics`, `branch_day_metrics`, `biz_day_metrics`
   ...
   - Если в окне нет ни одного дня с активностью, рейтинг становится `0`, но при инициализации устанавливается дефолтный рейтинг `50.0`
   - **Справедливость**: рейтинг не падает из-за выходных или каникул, учитываются только рабочие дни с реальной активностью

2. **Долгие паузы в работе:**
   - Если бизнес/филиал/сотрудник не работал более 30 дней (окно расчета), рейтинг постепенно "стареет" и может упасть до 0
   - При возобновлении работы рейтинг пересчитывается на основе новых метрик
   - **Рекомендация**: для долгих пауз (более 60 дней) владелец может вручную установить рейтинг через админ-панель или дождаться накопления новых метрик
...
4. **Инициализация рейтингов:**
   - При первом запуске системы или добавлении нового бизнеса/филиала/сотрудника устанавливается начальный рейтинг `50.0`
   - Если есть исторические данные, выполняется пересчет метрик за последние N дней (по умолчанию 30) через функцию `initialize_all_ratings(days_back)`
   - Если после пересчета рейтинг остается `0` или `NULL`, устанавливается дефолтный `50.0`

5. **Рекомендации для владельцев:**
   - Регулярно проверяйте рейтинги через админ-панель `/admin/ratings-status`
   - При долгих паузах (отпуск, ремонт) рейтинг может временно снизиться — это нормально
   - После возобновления работы рейтинг восстановится в течение окна расчета (30 дней)
   - При необходимости можно вручную пересчитать рейтинги через админ-панель
