...
**Важно для тестового номера:** Тестовый номер может отправлять сообщения только на номера, добавленные в список тестовых получателей.

**Как добавить номер в список тестовых получателей:**
...
6. В открывшемся окне найди раздел **"Управление номерами телефонов"** (Manage phone numbers) или **"Тестовые получатели"** (Test recipients)
7. Добавь номер телефона получателя в формате: `+996XXXXXXXXX` (например, `+996500574029`)
8. После добавления номера попробуй отправить тестовое сообщение

**Альтернативный способ:**
1. Перейди в [WhatsApp Manager](https://business.facebook.com/latest/whatsapp_manager/phone_numbers/)
2. Выбери тестовый номер (15551533441)
3. Найди раздел для управления тестовыми получателями
4. Добавь номер телефона, на который нужно отправлять сообщения
...
- **HTTP 400 - (#131047) Re-engagement message**:
  - **Проблема:** Прошло более 24 часов с момента последнего ответа клиента. WhatsApp Business API позволяет отправлять обычные сообщения только в течение 24 часов после последнего сообщения от клиента.
  - **Решения:**
    1. **Инициируй разговор через API Testing:**
       - Перейди в Meta Developers → WhatsApp → Быстрый старт → Протестируйте API
       - Отправь тестовое сообщение на номер получателя
       - Это откроет 24-часовое окно для отправки сообщений
    2. **Используй шаблоны сообщений (Message Templates):**
       - Шаблоны позволяют отправлять сообщения вне 24-часового окна
       - Шаблоны должны быть созданы и одобрены WhatsApp
       - Создай шаблон в WhatsApp Manager → Шаблоны сообщений
    3. **Для тестового номера:** Добавь получателя в список тестовых получателей
